...
┌─────────────────────────────────────────────────────────┐
│                    📊 数据源层                            │
│  养基宝 API  │  东方财富 API  │  天天基金  │  Push2 行情   │
└──────────────────────┬──────────────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────────────┐
│                  🔗 数据获取层 (data-layer.js)             │
│  超时重试 · 多源 fallback · 请求合并 · 缓存               │
└──────────────────────┬──────────────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────────────┐
│              🧠 分析引擎层                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │ Signal Engine │  │  7-Agent LLM │  │ Fund Scoring │  │
│  │ MA/回撤/止损  │  │  并行分析协作 │  │  五维量化评分 │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
└──────────────────────┬──────────────────────────────────┘
                       ▼
┌─────────────────────────────────────────────────────────┐
...
...
─────────────────────────────────────────────────────────────────────────────────────────
  趋势:MA排列 | 动量:多周期收益 | 资金:主力流向 | 回撤:风控 | 板块:行业热度
```

### 邮件报告 — 操作指令 + AI 解读

每天 14:30 自动发送，包含大盘快照、止损/补仓指令卡片、持仓概览、AI 投资洞察。移动端邮箱开箱即读，无需缩放。

### Web 看板

`http://localhost:3848` — 大盘实时行情 + 持仓总览 + AI 投资助理在线对话。
...
| `ci-test.yml` | Push / PR | 全项目语法检查 + 单元测试 |
...
系统在多处做了容错和降级设计，确保在数据源异常时仍能正常运行：

| 机制 | 实现 | 说明 |
|------|------|------|
| HTTP 超时 | 所有外部请求 10s 超时 | `fund-assistant.js`, `data-layer.js`, `yjb-api.js` |
| 自动重试 | 指数退避，最多 3 次 | 网络波动时自动恢复 |
| 多源 Fallback | 养基宝 → fund-config.json 降级 | API 不可用时仍能生成报告 |
| Promise.race | 6-8 秒超时保护 | 防止外部 API 拖死主流程 |
| 结构化日志 | Winston + 按天滚动 | `logs/` 目录，自动归档，7 天保留 |
| 错误边界 | 关键路径 try-catch + 错误日志 | 单只基金异常不影响全组合分析 |
| 语法+测试 CI | 每次 push 自动触发 | 218 项单元测试，防止回归 |

## 🤖 AI 协同开发

本项目采用 **AI 协同开发模式（Vibe Coding）**，在以下环节深度使用了大模型：

- **架构设计**：通过 Claude Code 进行 Multi-Agent 系统设计和模块拆分
- **代码生成**：核心引擎（Signal Engine / LLM Agent 编排）由 AI 辅助生成，人工审核验证
- **CI/CD 搭建**：GitHub Actions 工作流由 AI 根据项目结构自动配置
- **测试覆盖**：218 项单元测试的用例设计由 AI 辅助完成
- **MCP 集成**：探索了基于 Model Context Protocol 的多源数据接入方案

> 能够高效利用 AI 节点构建自动化 Workflow，是当下工程团队稀缺的"AI Agent 落地能力"。
