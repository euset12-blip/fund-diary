...
async function generateInsight({ fundProfiles, commands, market, summary } = {}) {
  ...
  try {
    const response = await chatCompletion(prompt, { maxTokens: 4000, thinking: true });
    return response;
  } catch (e) {
...
function chatCompletion(userPrompt, opts = {}) {
  const { system = '你是一个专业的基金经理，给出简洁有力的投资解读。', maxTokens = 2000, temperature = 0.7, thinking = false } = opts;
...
请基于以上原始数据独立分析，给出你的看法。`;
  ...
  const text = await chatCompletion(userPrompt, { system: systemPrompt, maxTokens: 4000 });
  return text;
}
