...
async function loadHoldings() {
  try {
    ...
    if (!holdings || holdings.length === 0) {
      console.warn('⚠️ 无持仓数据，请先登录养基宝：python yjb-api/yjb_tool.py --login');
      return null;
    }
    ...
    console.warn(`⚠️ 读取持仓失败: ${e.message}`);
    return null;
  }
}
  ...
  for (const arg of args) {
    if (arg === '--scan') mode = 'scan';
      ...
      if (!holdingsData || !holdingsData.holdings || holdingsData.holdings.length === 0) {
        console.log(color(COLORS.bold, '\n📋 当前持仓列表\n'));
        console.log('  ⚠️ 无持仓数据，请先登录养基宝：python yjb-api/yjb_tool.py --login\n');
        return;
      }
      const srcLabel = holdingsData.holdings._source || '养基宝';
      console.log(color(COLORS.bold, `\n📋 当前持仓列表 (来源: ${srcLabel})\n`));
